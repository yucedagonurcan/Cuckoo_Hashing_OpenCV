...
def AddNumberToHashTables(hash_tables, number, exclude_hash_tables):
    
    if CheckNumExistsInHashTables(hash_tables, number):
        print(f"{number} is already exists...")
        return True
    ...
    for current_try_hash_table in search_hash_tables:
        ...
        if reinsert_number is not None:
            ...
            return AddNumberToHashTables(hash_tables, reinsert_number, exclude_hash_tables)
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        if k == ord('p'):
            ...
            AddNumberToHashTables(hash_tables=hash_tables, number=add_number, exclude_hash_tables=[])
